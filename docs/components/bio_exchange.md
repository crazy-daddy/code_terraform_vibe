...
| --- | --- | --- |
| `order_id` | `string` | Id of a Bio Order from `orders()` (e.g. `"bio_order_01"`). |
...
| Name | Type | Description |
| --- | --- | --- |
| `item_id` | `string` | Candidate fragment id from an `ItemStack`. |
| `properties` | `any` | Exact property dict from the same `ItemStack`, or None for a propertyless sample. |
