...
| --- | --- | --- |
| `recipe_id` | `string` | Feed Maker recipe id |
...
| Name | Type | Description |
| --- | --- | --- |
| `recipe_or_id` | `any` | Feed recipe id, a `Recipe` object, or a dictionary or class instance with a string `id` field. The recipe must belong to this machine and be unlocked. |
...
- **Returns** Dict (`item_id` → count consumed per craft), or an empty dict if no recipe is set. Iterate it to stock the right life-forms + forage.

##### `.get_stockpile()`

Dict mapping each `item_id` currently in the input stockpile → its unit count. Read it to see what's loaded before crafting.

- **Returns** Dict (`item_id` → units currently in the input stockpile).
