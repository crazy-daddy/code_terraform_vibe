...
- **Seed supply** (`SeedSupplyController(SeedMakerController)`, reuses chamber/stock/output helpers): makes the seed with the largest deficit whose 3 blend forms are all in local stock. Deficit = `plant.seed_demand["now"]` − seeds at home (`total_stock()`: Inventory + home Warehouses). Finished seeds drain to a local Warehouse (`drain_port_to_storage()`), Inventory only as fallback. No fresh demand (older than `SEED_DEMAND_STALE_TICKS = 6000`) → `SEED_BUFFER_PER_SPECIES = 3` seeds per species. Life-form requests (§2i, requester `seed_maker`) = **base stock** (1 t per layout cell whose blend uses the form, from `plant.seed_demand["rotation"]`, i.e. one full replant) + open deficit, capped at `SEED_SUPPLY_STASH_T = 120` per form (room for a bulk replant). Forms outside the layout's blends get no request. Publishes `plant.recipes` for the Harvester (it has no `recipes()`). Restart: a chamber holding a known blend is combined, anything else is ejected.
...
- **Layouts**: three phases (`field_layout.py`, chosen by `harvester_planting.load_layout()`; `LAYOUT_VERSION = 4`; a stored full layout with an older version is rebuilt once, starter never):
  ...
  - **full** (after Field Automation; fixed 8 × 24, base pad `FULL_LAYOUT_BASE = "E13"`, always the middle): diversity garden with all 15 species in columns 1–`GARDEN_COLS = 5`, plus a fill, worked by Crop Automators (10 crowncap / 12 grandbloom). Only machines that give a kept plant a service it needs are reserved (`_needed_machines()`). `full_layout(chunks, fill)`: chunk 1 = the automators covering the garden (2 crowncap / 3 grandbloom); each further chunk adds the next automator left to right with the fill cells in its area. `validate()` + `provider_violations()` clean and 15 species for every chunk count of both fills. Fill (`FIELD_FILL = "crowncap"`, operator override Data Archive key `plant.field_fill`):
    - **phase 2, `crowncap`** (most of the game: Grandbloom only pays at Mk IV machines, >2,500 t/h water): `CROWNCAP_GARDEN` (columns 1–5, found by an offline search maximising the whole field's Forage/h: costs 14.6 Forage/h ×1 vs pure Crowncap; 1 Grow Lamp, 5 Sprinklers, 2 Dispensers, every one serving a plant) + solid Crowncap everywhere else except the base pad, lamp-lit cells (shade) and cells beside a spacer; clusters cut by a chunk edge are pruned (`_prune_clusters()`). 10 Crop Automators (C3, F3 in the garden + `CROWNCAP_AUTOMATORS` on rows C/F every 5th column) cover all 192 cells. 1.25 Forage per cell-hour with no machines, power or water; about 2× the life forms per Forage (a seed per 60 Forage vs 150). Chunk 1: 2 automators, 24 plants, ~440 Forage/h; chunk 9 (full): 170 plants, ~3,175/h.
  ...
  - Chunks: **crowncap fill = whole field** (`full_chunk_count()`, 9) right away, since it needs no machines, power or water. **grandbloom** is sized from demand: `chunks_for_demand(terraformer_demand(plant.terraformer), rules, fill)`, `forage_per_hour()` = Σ base/growth × diversity at Mk I; Terraformer use `TERRAFORMER_BATCH = {1: 1200, 2: 6600}` per `TERRAFORMER_CYCLE_H = 3`. The switch to full happens once; a full layout then only grows (same fill), re-checked every `LAYOUT_RECHECK_TICKS = 3000`. Full mode: no paving (the Harvester drives over machines); cells a deployed automator serves are its work; the Harvester plants garden cells no automator serves plus the first `HARVESTER_FILL_CHUNKS = 1` fill group(s) whose automator is not deployed yet (one Harvester cannot keep up with the whole fill, so the field grows chunk by chunk as automators arrive); seed demand (`seed_layout()`) covers the Harvester cells + automated cells. The Harvester harvests everything else mature.
...
- **Harvester loop** (`FieldKeeperController`), one task per step, cheapest route (heat) first: **build** (full layout only: deploy a field machine whose kit is at home, or plant an own cell, picked by `field_layout.work_order()` rank instead of heat: the garden row by row in a snake, then one group per fill Crop Automator left to right, its own cell first, then its area in a snake; first: each machine removes care work for good, and behind care it never ran live) → **care tour** ( a lapsed treatment stalls growth, a mature crop just waits; harvest-first starved care live) → harvest mature (then replant on the spot) → uproot a plant in the layout's way (rebuild, above) → plant open layout cell with its seed at home (collects a loose item there first) → **pave** a path cell (below) → collect loose items off the path (only at heat ≤ `ITEM_SWEEP_MAX_HEAT = 40`; only path cells keep their items, so seeds from an older layout's path are reused for paving, then collected back into Inventory) → wait in place (no trip back to base: costs heat, nothing needs the base). Care tour: once any treatment drops below `CARE_REFRESH_H = 4.0` h, every treatment below `CARE_BATCH_H = 12.0` h is renewed in one tour, so renewals line up. Any visit also renews everything below `CARE_BATCH_H` on that cell. **On the way**: every Harvester-own cell (`harvester_layout()`) a route passes through (not the target, nothing held) is harvested if mature and planted if open with its seed at home (`work_on_pass()`, called from `move_to()`): the next pass costs +1 heat instead of +7. Provider-covered = flag set with 0 manual time. Water refilled when < 1 t aboard. Failed plant → cell skipped `PLANT_FAIL_COOLDOWN_TICKS = 3000`. `inventory_full` on harvest → harvesting paused `INVENTORY_FULL_RETRY_TICKS = 3000`. Harvester actions retried `ACTION_RETRIES = 3` on busy/moving.
...
- **Field machines, Harvester side** (`harvester_machines.py` `HarvesterMachinesMixin`): deployed machines from `home.harvesting_machines()` (type + position, once per step; fallback `Cell.status == "provider"`). **Kit order**: standing Fabricator order `fabricator.upgrade_orders["field_keeper"]` for the Grow Lamp / Sprinkler / Dispenser kits still missing on reserved cells (only kinds in `deployables()`, re-read every `DEPLOYABLES_REFRESH_TICKS = 3000`), a stock target capped at `KIT_STOCK_CAP = 10` per kit so Warehouses don't fill with kits. Full layout: its reserved cells. Starter: **pre-order** for `full_layout(desired chunks)` once the Plants Sensor reads ≥ `PREORDER_MIN_KM2 = 550000` (Field Automation at 620,000). **Crop Automator kits** are Shop items (catalogue price, fallback `AUTOMATOR_PRICE_FALLBACK = 30000`): full layout only, one bought per publish while one is missing and none is at home, if credits ≥ price + `AUTOMATOR_CREDIT_RESERVE = 100000`; the missing count is `plant.status[...]["automators_wanted"]`. **Deploy** (full layout only): kit at home → drive there, collect a loose item, `stage()` the kit, `deploy()`. Failed cell skipped `DEPLOY_FAIL_COOLDOWN_TICKS = 3000`. A new machine has no script until scripts_sync fills its slot.
- **Crop Automator** (`crop_automator.py` `CropAutomatorController`, thin `harvesting/crop_automator.py`), every `POLL_INTERVAL_S = 10`, full layout only: consumes job results (the 50-result inbox pauses the machine when full); owns the layout cells in its 5 × 5 area that no nearer deployed automator reaches (Chebyshev, then Manhattan, then sector id: every cell exactly one owner); queues `harvest()` for its mature crops (status `mature` or growth ≥ 1.0) and `plant()` for its open cells once the machines beside the cell give every care service the species needs (automators only apply Fertilizer/Growth Accelerant); seeds in the input are netted against queued plant jobs (`queued_jobs_info()` → committed seeds per id), and only when none are spare are `SEED_PULL_BATCH = 10` loaded at once (`take_item()`, Inventory then home Warehouses); keeps the queue ≤ `QUEUE_LIMIT = 45`; failed cell skipped `JOB_FAIL_COOLDOWN_TICKS = 3000`. **Blocked head job** (`unblock_queue()`, FIFO executor would freeze): `no_seed` → load one seed, else `cancel_job()`; `output_full` → wait (debug log only). **Forage stays in the output** (holds 50,000; no drain to Warehouses) so Warehouse auto-loaders stay free; a clogged automator is accepted over clogged Warehouses. **Drain priority** (`storage.crop_automator_forage()`, used by `take_item()` via `_holder_candidates()`): clogged automators (`status() == "output_full"` or output ≥ `CROP_AUTOMATOR_CLOG_FRACTION = 0.9` × `CROP_AUTOMATOR_OUTPUT_CAP = 50000`) → Inventory → Warehouses → other automators; within the automator groups garden ones first (they carry the diversity bonus; `garden` flag = automator column ≤ `GARDEN_COLS`, from `plant.automators` telemetry), then most Forage. Automators are found by `harvesting_machines("crop_automator")` type id, never by id spelling. `logistics_requests.outpost_stock()`/`outpost_free_stock()` count automator Forage at home (`crop_automator_forage_total()`). **Seed demand fallback** (`refresh_seed_demand_if_stale()`): if `plant.seed_demand` is ≥ 3000 ticks old (Harvester offline), the first deployed automator republishes it from the layout (`rotation` = cells per seed, `now` = clamp(cells ÷ 6, 3, 30) per seed). No jobs while in `power.shedded`. Loose items on its cells are swept by the Harvester. Telemetry `plant.automators`. Draw 60 W.
...
- **Seed demand** `plant.seed_demand` (every `PUBLISH_INTERVAL_TICKS = 600`): `now` = open layout cells + layout crops at growth ≥ `SEED_PREFETCH_GROWTH = 0.75` or mature + a per-seed buffer `max(SEED_BUFFER_PER_SPECIES, clamp(rotation ÷ 6, 3, 30))` (scales with how many cells use the seed); `rotation` = cells per seed in `seed_layout()` (full layout: the Harvester's own cells + automated cells).
...
**Updaters must be pure:** a `transaction()` updater may only compute the new value (mutating a closure list like `claimed = [False]` is fine). Any log/print call inside it gets the whole write rejected — `lib/archive.py`'s wrapper just returns `False`, so it fails silently. Collect messages in a `notes` list and log them after the call; check the return value where a failed write matters (claims, telemetry). Found 2026-09-24: drill telemetry and biosite claims had silently stopped persisting.
...
- `plant.automators`: `{machine_id: {sector, status, queue, cells, mature, open, waiting_machines, forage, garden, tick}}` Crop Automator telemetry — see §1k. `forage` = Forage in its output; `garden` read by `storage.crop_automator_forage()` for drain priority. Entries unrefreshed for `STATUS_STALE_TICKS = 36000` pruned by the next publishing automator.
