...
| Drone Depot (cargo logistics endpoint) | `drone_depot.py` — see §2h; also buffers life forms in a local Warehouse (one stack per form) |
...
- **Requests**: every `REQUEST_REFRESH_TICKS = 600`, forms still in ≥1 open triple requested at `SEED_STASH_TARGET_T = 10` t via `logistics_requests.set_requests(outpost, "seed_maker", ...)` (§2i). Cleared once all 15 known or no open triple left.
- **Output**: seed sent to `"inventory"` (planting starts there), else `drain_port_to_storage()`.
- Telemetry `seed_maker.status` (§4). Stage B (plant/harvest) not implemented.
...
- **Pickups** `logistics.pickups` (§4): `reserve_pickup()` per planned/loaded item, `release_pickups(vehicle)` after unloading; stale after `PICKUP_STALE_TICKS = 36000`. Kept separate from `mining.reserved_yield` (that one feeds home ore demand).
- **Reverse hauler**: unchanged Pioneer template, hauler role (no role module) with `DESTINATION_OUTPOST_ID` in `PULL_DESTINATION_WILDCARDS = ("*", "any", "%")` → `run_pull_loop()`; `HOME_BASE` = where it parks = where requests are delivered. Per cycle: cargo aboard → deliver home first; else live deficits at home → `_plan_pull_route()`: nearest-neighbour chain of other outposts whose Warehouse (+ Inventory if home) stock minus that outpost's own request target covers a deficit, largest deficits first, up to `PULL_MAX_STOPS_PER_TRIP = 3` stops / cargo capacity. Stops after the first only chained if direct leg ≤ `PULL_CHAIN_MAX_DETOUR_RATIO = 0.75` × (prev → home → next), so it never drives past home; skipped stops wait for the next trip. Trip skipped if planned < `min(PULL_MIN_LOAD_UNITS = 10, total deficit)`. Legs via `drive_with_recharge()` (already refuses a leg that can't reach a charging station afterwards); recharge at stops that have a station; unreachable stop → head home with what's aboard.
- **Source retention**: `retain_amount(item, outpost_id)` = own request target if this outpost requests it, else `LIFEFORM_STASH_CAP_T = 25` while any other outpost still lacks it, else 0. Used by Drone Depot staging (§2h) and Essence Liquifier feed (§1h). Vehicles load from Warehouses only (Depot buffer staging, §2h, puts life forms there).
...
- `smelter.diag.<smelter_id>`: **retired 2026-09-23**. `SmelterController.__init__` deletes leftover key. Per-step outcome now debug()-only via `log_outcome()`: `shedded`, `no_demand`, `no_ore`, `ore_reserved_for_dock`, `buffer_full`, `fair_share_capped`, `took`, `busy_all_sources` (harmless), `busy_starving` (real lost time), `recipe_switch`, `output_blocked`, `demand_covered_by_peers`.
...
- `logistics.pickups`: `{"pull:<vehicle>:<dest>:<item>": {"vehicle", "dest", "item_id", "units", "tick"}}` in-flight reverse-hauler pickups — see §2i.
...
- `seed_maker.status`: `{maker_id: {state, found, tried, open, saturated, last, tick}}` — see §1i.
