...
**Key convention:** one shared dict per concern `{entity_id: value}` via `archive.transaction()`, never one key per entity — archive has fixed key-count cap (CLAUDE.md rule 7). Per-entity keys below (`vehicle.mission:<name>`, `drone.mission:<name>`, ...) are legacy, pending consolidation (TODO.md).
