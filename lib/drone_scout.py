...
class DroneScoutMixin:
    ...
    def run_scout_loop(self, poll_interval=5.0):
        tree = TreeConsole()
        print(f"Drone Scout Controller ({self.name}) online. Home outpost biome: {self.home_biome or 'unknown'}.")
        while True:
            try:
                if self.is_stranded():
                ...
                if not candidates:
                    tree.debug(f"[{self.name}] No unscanned POI candidates this cycle.")
                ...
                for candidate in candidates:
                    ...
                    tree.debug(f"POI {candidate}: {budget['total_required_wh']:.1f} Wh required, achievable={budget['is_achievable']}.")
                    if budget["is_achievable"]:
                        target = candidate
                        break
                if not target:
                    tree.debug(f"[{self.name}] {len(candidates)} candidate(s) found but none reachable on current battery.")
                    self.publish_telemetry("IDLE_OUT_OF_RANGE")
                    sleep(30.0)
                    continue

                tree.print(f"[{self.name}] Flying to POI {target} to scan.")
                self.publish_telemetry("OUTBOUND", f"poi_{target[0]}_{target[1]}")
                if not self.fly_to(target[0], target[1], precision=1.0):
                    print(f"[{self.name}] Could not safely reach POI {target}; will retry.")
                    sleep(poll_interval)
                    continue
                ...
                if res.status == "ok":
                    ...
                    if scan is not None and getattr(scan, "is_empty", False):
                        ...
                        tree.debug(f"[{self.name}] {target} confirmed empty; cached to skip on future cycles.")
                    else:
                        ...
                        print(f"[{self.name}] Biosite found at {target}: {types_found}.")
                    self.publish_telemetry("SCANNED", f"poi_{target[0]}_{target[1]}")
                else:
                    print(f"[{self.name}] Scan at {target} notice: {res.status} - {res.message}")

                sleep(poll_interval)
            except Exception as e:
                print(f"[{self.name}] Scout loop exception: {e}")
                sleep(5.0)
