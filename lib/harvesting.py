...
class HarvesterController:
    ...
    def __init__(self, harvester):
        ...
        self.log = TreeConsole(module="harvesting")
    ...
    def store_held_if_any(self):
        ...
        if held and held != "":
            ...
            if res.status == "ok":
                self.log.print(f"[{self.name}] Successfully stored '{res.item_id}' to Inventory.")
                return True
    ...
    def move_to(self, target_sector):
        ...
        if not path:
            self.log.level("warn").print(f"[{self.name}] No valid path found from {curr_pos} to {target_sector}!")
            return False
    ...
    def collect_at_current(self):
        ...
        if res.status == "ok":
            ...
            self.log.print(f"[{self.name}] Collected '{item_name}' (value: {val}) at {self.get_position()}.")
            self.store_held_if_any()
            return True
            ...
            self.log.level("warn").print(f"[{self.name}] Collect notice: {res.status} - {res.message}")
            return False
    ...
    def harvest_at_current(self):
        ...
        if res.status in ["ok", "partial"]:
            self.log.print(f"[{self.name}] Harvested crop at {self.get_position()}: {res.status} - {res.message}")
            return True
        elif res.status == "overheated":
            self.cool_down(self.HEAT_RESUME_LEVEL)
            return self.harvest_at_current()
        else:
            self.log.level("warn").print(f"[{self.name}] Harvest notice: {res.status} - {res.message}")
            return False
